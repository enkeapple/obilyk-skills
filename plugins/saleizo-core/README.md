...
- **[adopting-framework](skills/setup/adopting-framework/SKILL.md)** — Use right after the saleizo-core plugin is installed into a fresh consumer repo, to do the per-consumer bootstrap the plugin…
...
- **[auditing-hooks](skills/setup/auditing-hooks/SKILL.md)** — Use to find drift between hook scripts on disk and their wiring in the harness hook configuration (each settings.json hooks block and each plugin hooks.json) — orphan scripts, dangling wiring, broken symlink…
