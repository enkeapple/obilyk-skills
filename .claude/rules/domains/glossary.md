...
- Anytime you read or edit skill source under `plugins/*/skills/**`, `.claude/rules/**`, `hooks/**` (guard hooks surfaced via `.claude/hooks/**`; routing/metric/session/quality hooks in `plugins/saleizo-controls/hooks/`), or `.claude/skills-routing.json`.
...
| --- | --- | --- | --- | --- |
| 1 | **skill** | source `plugins/<kit>/skills/<name>/SKILL.md` (+ `references/*.md`, `assets/*.md`); flat — no category dir; discovered via the installed marketplace plugin | invoked via the `Skill` tool | a routable capability; `name:` MUST equal the directory name and the routing key |
...
| 3 | **hook** | routing/metric/session/quality hooks ship in the `saleizo-controls` plugin (`plugins/saleizo-controls/hooks/`), wired by its `hooks.json`; guard hooks remain vault-local in `hooks/guards/`, surfaced via `.claude/hooks/` symlinks, wired by root `settings.json` | runs on tool events | a gate/logger: routing (`detect-bypass`, `skill-gate`, `log-skill-usage`), metric (`friction-log`), guards (`security-guard`, `bash-read-guard`, `read-guard`, `edit-write-guard`), quality (`quality`), session (`reset-turn-budget`, `token-guard`, `lessons-nudge`); authored/edited test-first via `writing-hooks` (fixture-execution is its RED/GREEN) |
