...
- A repo that routes `writing-specs`/`writing-plans` whose CLAUDE.md declares no single design-docs home, or two competing ones (`specs/` and `docs/specs/`) — the output path is then a coin-flip; the doc must name exactly one location per artifact (the `docs/specs/`+`docs/plans/` defaults unless the repo keeps one elsewhere).
...
| "It uses writing-specs but doesn't say where specs go — the skill has a default." | The skill defaults only when it detects no convention; once a repo has two design-docs dirs the detection is a coin-flip. A chain repo's CLAUDE.md must name one home per artifact; a missing/multi-valued convention is drift. |
