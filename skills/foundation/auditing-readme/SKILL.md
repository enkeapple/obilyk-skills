...
**Every row is a derived claim, not prose to skim.** Re-derive the catalog from frontmatter via the [catalog-derivation contract](./references/catalog-derivation.md) — the SAME contract `bootstrapping-readme` writes from — and compare. Eyeballing "looks complete" is not an audit; a row that disagrees with disk is drift, not a detail. In particular, derive **kind from routing, never from `disable-model-invocation`** (the trap a quick read falls into).
...
2. **Re-derive the catalog from disk** per the [catalog-derivation contract](./references/catalog-derivation.md) (ROOT, discovery, category, kind from routing, description algorithm, ordering).
3. **Compare every criterion** (see Report) — do not stop at the first drift; check all six across every skill.
...
1. **Findings** — table: criterion → what disk shows → status, over the six criteria:
   1. markers well-formed; 2. every SKILL.md appears exactly once; 3. grouping + ordering match disk; 4. each description matches the derived one; 5. each row link resolves; 6. each kind matches the routing-derived kind.
...
- Deriving kind from `disable-model-invocation` instead of routing.
